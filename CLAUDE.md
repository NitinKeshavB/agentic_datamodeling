...
All jobs use **serverless compute** (`environment_key: serverless`, `spec.client: "1"`).
...
- `postgresql://` and `postgresql+asyncpg://` connection strings are silently rewritten to `postgresql+psycopg2://` by `JDBCConnector._normalise()` — psycopg2 is required for `sslmode` support.
